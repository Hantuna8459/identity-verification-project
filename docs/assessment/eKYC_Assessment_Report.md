...
| Cấu trúc tài liệu | **Phần A** Executive Summary · **Phần B** eyePass — đặc tả tham chiếu · **Phần C** V-ID-eKYC — đánh giá năng lực · **Phần D** Gap & Traceability Matrix (evidence đầy đủ, từng dòng capability) · **Phần E** Risk & Decision Log |
...
### V-ID đã đáp ứng/vượt yêu cầu eyePass, kiểm chứng được (chọn lọc)

Chỉ liệt kê những gì đạt `Implemented`/`Hardened` + `Source-reviewed`/`Test-verified` — nghĩa là đọc code hoặc chạy test thật, không dựa vào tài liệu tự báo cáo.

- **Tách execution/review-signal/decision** — score không còn đóng vai verdict, có test tự động. *(C04.1)*
- **Manual-review-only** — không có đường auto-approve/reject nào, hard-coded, test phủ cả happy path lẫn failure path. *(C04.2)*
- **QR handoff một lần, chống replay** — token dùng một lần, test xác nhận claim lần hai bị từ chối 401. *(C01.1)*
- **Face-match similarity/aggregation** — median trên tối đa 12 frame, test resist một frame lạc quan. *(C03.1)*
- **PII minimization trong response public** — `SessionPublic` không có PII/raw OCR/model score. *(C05.1, C07.1)*
- **Evidence encryption + opaque storage key + chống path-traversal** — AES-GCM, 2 test trực tiếp. *(C07.2)*
- **Model governance hai lớp (approval) + manifest checksum + offline runtime fail-closed** — 3/4 dòng C06 đạt `Hardened`/`Test-verified`. *(C06.1–C06.3)*
- **Passport MRZ check-digit ICAO đầy đủ** — test-covered. *(C02.3)*
- **Xác thực cho actor có quyền cao (tích hợp V-ID, reviewer)** — shared-secret trên header, từ chối `401` khi thiếu/sai. *(C08.1)*
- **Test tự động cho model/AI pipeline và luồng e2e chính** — 17 test cho OCR/MRZ/liveness/face-match, 8 test e2e qua `TestClient`; toàn bộ suite 40 test pass khi tự chạy `uv run pytest -q`. *(C09.2, C09.3)*
- **Release gate M7 nhiều điều kiện + roadmap có sơ đồ phụ thuộc + ADR cho quyết định đã chốt** — đọc trực tiếp và xác nhận cấu trúc tồn tại thật. *(C10.2, C10.1, C10.4)*

### Năng lực V-ID có mà eyePass không hề đề cập (`NGOÀI YÊU CẦU EYEPASS`)

- Session/state machine tường minh với terminal state. *(C01.2)*
- Active liveness (challenge quay đầu), deepfake/replay/camera-injection heuristics, voice challenge, lip-sync — **nhưng tự dự án đã ghi nhận các signal này chưa được benchmark/calibrate thành bằng chứng chống spoof production**; xem quyết định gộp D07 trong decision log. *(C03.3–C03.6)*
- Review queue có cấu trúc (`/reviews`, reason codes). *(C04.3)*
- RBAC granularity cho reveal/decrypt/export/delete evidence — đã thiết kế (M6), chưa triển khai; xem D02. *(C04.4)*
- API versioning theo path prefix, polling trạng thái phiên. *(C05.2, C05.4)*
- Approval hai lớp cho model/provider, offline runtime fail-closed. *(C06.2, C06.3)*
- Purge có audit trail, audit trail cho vòng đời evidence. *(C07.3, C07.4)*
- Unit/contract test cho tầng abstraction (provider registry, fallback, governance). *(C09.1)*
- Roadmap có sơ đồ phụ thuộc milestone; cơ chế ghi quyết định-đã-chốt kiểu ADR; bước đầu của một decision log có cấu trúc (chính là Phần E của báo cáo này — không phải năng lực có sẵn từ trước, xem C10.3). *(C10.1, C10.3, C10.4)*

### Rủi ro/gap cần owner xác nhận ưu tiên (P1, không có P0)

Không có P0. Bảng dưới là toàn bộ P1 đang mở, chi tiết + đề xuất owner nằm ở Phần E §2:

| ID quyết định | Chủ đề | Dòng ma trận |
|---|---|---|
| D01 | Webhook dispatch chưa tồn tại (chỉ có field lưu trữ + allowlist, không có lệnh gọi HTTP ra ngoài nào) | C05.5 |
| D02 | RBAC granularity cho reveal/decrypt/export/delete đã thiết kế (M6), chưa triển khai | C04.4 |
| D03 | CCCD chưa có structured field extraction (Passport đã có) | C02.2 |
| D04 | Document quality gate & reason-code recapture (M3) `NOT_STARTED` | C02.4 |
| D05 | Idempotency key đã thiết kế nhưng chưa nối vào schema hiện tại | C05.3 |
| D10 | `VID_CLIENT_KEY`/`REVIEWER_TOKEN` fallback về giá trị mặc định công khai khi thiếu cấu hình, không có gate theo `environment` | C08.2 |
| D11 | `mypy` có 19 lỗi type thật (một phần nằm trong `purge_due()`, cùng vùng code C07.3 đã thiếu test), vi phạm gate M7 | C09.4 |

### Quyết định phạm vi cần owner (không phải mức độ ưu tiên kỹ thuật)

Đây là những câu hỏi "có làm hay không", không phải "làm cái gì trước" — chi tiết ở decision log §3:

| ID quyết định | Câu hỏi | Dòng ma trận |
|---|---|---|
| D06 | End-user có được tự sửa/skip-validate trường OCR trước khi submit không? (eyePass cho phép; V-ID chưa quyết) | C02.6 |
| D07 | Threshold + các signal sinh trắc học nâng cao: cùng phụ thuộc M4/M5 chưa benchmark — xử lý như một quyết định gộp | C04.5, C03.2–C03.6 |
| D08 | Mốc thời gian cho dataset registry record (X2/M4) | C06.4 |
| D09 | Retention matrix production (Legal/DPO + business owner chốt) | C07.5 |
| D12 | Có nên phân vai trò owner theo chức năng (security/compliance/AI...) trước khi mở rộng đội, hay giữ một owner chung tới khi nào | C10.5 |

### Bước tiếp theo

Owner cần xác nhận thứ tự ưu tiên D01–D05, D10, D11 (decision log §2) và trả lời các câu hỏi phạm vi D06–D09, D12 (§3). Trong đó D10 (secret fallback) và D11 (lỗi mypy) đáng làm trước vì rẻ — chạy lại đúng lệnh đã dùng để tìm ra chúng là xác nhận được ngay, không cần điều tra gì thêm. Và trước khi đưa báo cáo này ra ngoài phạm vi nội bộ dự án, cần bố trí calibration với một assessor thứ hai.
...
- **RBAC granularity cho reveal/decrypt/export/delete** (`Không đề cập`) — không có tài liệu nào đề cập. *(→ C04.4)*
...
- Retry/recapture chia làm hai: phần retry cơ bản `Implemented`/`Source-reviewed`; quality-gated recapture (blur/glare/reason code) mới `Designed`/`Doc-only`, M3 `NOT_STARTED`. *(→ C01.4)*
...
- Phạm vi loại giấy tờ: `Implemented`/`Source-reviewed`. CCCD chạy YOLO11 + RapidOCR PP-OCRv6, Passport chạy TD3 OCR + MRZ parser, M2 = `DONE`. NFC/chip là `OUT_OF_SCOPE` MVP có chủ đích; CMND/military ID chưa trong scope. *(→ C02.1)*
...
- Document quality gate còn `Designed`/`Doc-only` — đặc tả đầy đủ ở M3 (reason code, state `RECAPTURE_DOCUMENT`) nhưng M3 `NOT_STARTED`, capability `document_quality` chưa có provider nào đăng ký. *(→ C02.4)*
...
- Fixture/benchmark OCR: `Conceptual`/`Self-reported`. M4 có đặc tả dataset/metric cụ thể (MIDV-2020, DocXPand-25k, CER/WER...) nhưng chưa bắt đầu. *(→ C02.7)*
...
- Review queue phần core (`/reviews`, decisions, reason codes) đã `Implemented`/`Source-reviewed`; assignment, idempotency, `ESCALATE` thì còn `Designed`/`Doc-only`, M6 `NOT_STARTED`. *(→ C04.3)*
- RBAC granularity cho reveal/decrypt/export/delete mới `Designed`/`Doc-only` — mask mặc định đã đúng nhưng phân quyền chi tiết theo M6 chưa triển khai. *(→ C04.4)*
- Threshold versioned/approved: `Conceptual`/`Self-reported`. Seed threshold đang `evaluation_only`, lifecycle đã thiết kế nhưng M5 `NOT_STARTED` — việc chưa phê duyệt cho production là quyết định chủ đích. *(→ C04.5)*
...
Mục còn lại — dataset registry — dừng ở `Designed`/`Doc-only`: schema/quy trình 5 bước đã chốt (M0 `DONE`), nhưng dataset record cụ thể chủ động để lại workstream X2/M4. *(→ C06.4)*
...
- Quy trình incident/change control: `Conceptual`/`Doc-only`, chỉ có một dòng ý định trong roadmap (M7), chưa có runbook thật. *(→ C08.5)*
...
- Roadmap có sơ đồ phụ thuộc: `Implemented`/`Source-reviewed`, M0 làm nền cho M1 và nhánh M2→M3→M4→M5→M6→M7, X1/X2 chạy song song. *(→ C10.1)*
- Release gate nhiều điều kiện: `Implemented`/`Source-reviewed`, M7 liệt kê 8 điều kiện phải đạt đồng thời. *(→ C10.2)*
- Theo dõi quyết định còn mở có cấu trúc: nền tảng thì `Designed`/`Source-reviewed` — trước đợt đánh giá này chỉ có hai danh sách gạch đầu dòng rải rác (`AGENTS.md`, roadmap §7), không ID/trạng thái/owner. Bản có cấu trúc thật sự (Phần E) là sản phẩm của chính đợt đánh giá này. *(→ C10.3)*
...
Vài điểm đáng nêu ra sau khi chấm hết 10 domain. Không có dòng nào ở `Absent` mà thiếu lý do — C02.5 (multi-doc detection), C02.6 (end-user self-edit), C08.4 (logging/monitoring), C09.5/C09.6 (CI, frontend test) đều đã gắn `GAP` hoặc câu hỏi phạm vi trong ma trận, không phải bị bỏ sót giữa chừng. `Hardened`/`Test-verified` tập trung nhiều nhất ở C04, C06, C07 — decision separation, model governance, evidence encryption/opaque-key là ba mảng có test trực tiếp dày nhất. Khoảng cách lớn nhất giữa thiết kế và triển khai rơi vào M3 (document quality gate), M4 (benchmark), M5 (threshold), M6 (RBAC/review queue mở rộng) — tất cả `NOT_STARTED` nhưng đều có đặc tả rõ trong kế hoạch, và tự dự án đã liệt kê các milestone này là điều kiện cho `MVP feature-complete`, nên không tính là gap ẩn.
...
| Thuộc tính | Giá trị |
|---|---|
| Trạng thái | **DRAFT — single-assessor first pass, chưa calibration** |
| Dùng cùng | [`EKYC_COMPARATIVE_ASSESSMENT_RUBRIC.md`](../EKYC_COMPARATIVE_ASSESSMENT_RUBRIC.md) v3.0, [`EKYC_COMPARATIVE_ASSESSMENT_BASELINE.md`](../EKYC_COMPARATIVE_ASSESSMENT_BASELINE.md) |
| Mô hình chấm | Hai thang không dùng chung (V-ID: maturity + confidence; eyePass: mức nêu trong đặc tả) — định nghĩa đầy đủ ở Phụ lục đánh giá. |
| Phạm vi đã chấm | C01–C10 (toàn bộ 10 domain, 56 dòng capability) |
| Phạm vi chưa chấm | Không còn — vẫn cần calibration với assessor thứ hai |
| Assessor | 1 (Claude, phiên làm việc với owner, một phần qua các fork con chấm song song từng domain) — rubric không bắt buộc 2 assessor độc lập, nhưng mọi `P0`/`P1` vẫn cần owner xác nhận trước khi chốt (rubric §10) |
| Nguồn V-ID | Quản trị, thiết kế chính (đọc toàn văn), thiết kế rút gọn, hợp đồng capability, kế hoạch (đọc toàn văn), trạng thái (đọc toàn văn) — xem quy ước gọi tên bên dưới — cộng source (`backend/app`, `backend/tests`, `backend/ai_modules`, `frontend/app`, `frontend/components`) |
| Nguồn eyePass | BRD, business rules, flow diagram, kiến trúc, API doc, SDK doc, UAT/handover checklist, test-case workbook trong bộ tài liệu bàn giao (`~/Documents/Tài liệu bàn giao eKYC/`) — không có source/test truy cập được, nên chỉ dùng để rút ra mức độ nêu trong đặc tả (rubric §4), không phải maturity |

**Quy ước gọi tên tài liệu V-ID** (tránh lặp filename trong mọi ô evidence — chi tiết đầy đủ ở rubric §2):

| Gọi tắt | Tài liệu | Vai trò |
|---|---|---|
| *quản trị* | `AGENTS.md` | Hiến chương quản trị, nguyên tắc bắt buộc, giai đoạn hiện tại |
| *thiết kế chính* | `EKYC_FLOW_DESIGN.md` | Kiến trúc/state machine/data model/API contract, target design đang chờ review |
| *thiết kế rút gọn* | `EKYC_FLOW_DESIGN_SIMPLIFIED.md` | Bản rút gọn của thiết kế chính — có một số chi tiết (vd. QR handoff §6.1–6.3) cụ thể hơn thiết kế chính; dùng song song, không thay thế |
| *hợp đồng capability* | `M0_CONTRACT_GOVERNANCE_BASELINE.md` | Schema/contract cho từng capability |
| *kế hoạch* | `PROJECT_ROADMAP.md` | Đặc tả và điều kiện hoàn thành theo milestone (M0–M7), thường cụ thể hơn thiết kế chính cho phần chưa `DONE` |

`IMPLEMENTATION_STATUS.md` cố tình không nằm trong bảng gọi tắt này và không
được trích dẫn làm evidence ở bất kỳ dòng nào trong Phần D (xem `AGENTS.md`,
mục "Thứ tự ưu tiên tài liệu") — mọi khẳng định phải trích dẫn source/test thật.

### 1. Ghi chú phương pháp

Rubric đã chuyển từ thang chung `L0`–`L4` sang hai thang tách biệt (v3.0) —
xem Phụ lục đánh giá cho định nghĩa và lý do đổi. Hai lần chấm lại vì đọc chưa
đủ toàn văn nguồn đã được sửa trước khi chốt bản này: C04.4 (đọc thêm kế hoạch
milestone M6 và source `review_detail`) và C01.1 (đọc thêm UAT checklist, phát
hiện luồng QR cross-device mà flow diagram không vẽ). Các dòng tương ứng dưới
đây đã phản ánh bản đã sửa.

### 2. Domain C01 — Luồng và UX

| ID | Expectation | eyePass — Mức nêu trong đặc tả / Trích dẫn | V-ID-eKYC — Maturity / Confidence / Evidence |
|---|---|---|---|
| C01.1 | Cross-device handoff: tạo token dùng một lần, chống replay, revoke, desktop theo dõi trạng thái | `Nêu chưa đầy đủ` — UAT checklist, "Full Onboarding \| Verify on mobile \| Get QR code \| Scan the QR Code" (R49-R52): xác nhận có luồng quét QR để chuyển từ desktop sang mobile, nhưng không mô tả cơ chế bảo mật (hết hạn, dùng một lần, chống replay, revoke). Không tài liệu nào khác trong bộ bàn giao (BRD, business rules, API doc) nhắc lại luồng QR này. | Thiết kế rút gọn §6.1-6.3 mô tả rõ: QR chỉ chứa URL + token ngẫu nhiên dùng một lần, không chứa PII, có vòng đời token và bốn endpoint riêng (`POST /sessions/{id}/handoffs`, `POST /handoffs/claim`, `GET /sessions/{id}/handoff-status`, `POST /sessions/{id}/handoffs/{id}/revoke}`). Implementation khớp thiết kế — `backend/app/api.py` dòng 167-270 (`create_handoff`/`claim_handoff`/`handoff_status`/`revoke_handoff`). Test `test_full_cccd_qr_capture_and_manual_review` (`backend/tests/test_ekyc_flow.py:43-51`) xác nhận claim thành công và replay cùng token bị từ chối 401. `Implemented`/`Test-verified`, dù chưa thấy test riêng cho expired-token hay revoke-rồi-claim nên chưa đủ để gọi `Hardened` toàn phần. |
...
| ID | Status (V-ID) | Nhận định | Ưu tiên | Ghi chú |
|---|---|---|---|---|
| C01.1 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | — | eyePass chỉ nêu tên tính năng QR, không có chi tiết bảo mật. V-ID có thiết kế, implementation, và test cho negative case cốt lõi (replay); expiry/revoke chưa có test riêng, để P2 theo dõi. |
| C01.2 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không có khái niệm state machine — năng lực V-ID tự thêm. State machine hiện tại đơn giản hơn thiết kế mục tiêu 13-trạng-thái, nhưng đó là gap nội bộ, không liên quan gì eyePass. |
| C01.3 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | `P2` | Test-covered. eyePass có thêm flow "Auto Detect Document Type" mà V-ID chưa có, nhưng đó là ghi nhận riêng, không hạ nhận định ở đây. |
| C01.4 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` (retry cơ bản) · phần quality-gated recapture: `GAP` | `P2` | Retry cơ bản đọc trực tiếp từ source, đủ điều kiện đáp ứng. Quality-gated recapture (M3) đã thiết kế rõ nhưng chưa triển khai và không ai giải thích vì sao hoãn, nên `GAP` chứ không phải `DEFERRED_BY_DESIGN`. |
| C01.5 | `IN_SCOPE` | `GAP` | `P2` | eyePass mô tả rất cụ thể; V-ID mới dừng ở ý định thiết kế chung, chưa có ghi nhận quyết định nào để gọi đây là hoãn có chủ đích. |

### 3. Domain C02 — Giấy tờ/OCR/MRZ

| ID | Expectation | eyePass — Mức nêu trong đặc tả / Trích dẫn | V-ID-eKYC — Maturity / Confidence / Evidence |
|---|---|---|---|
| C02.1 | Phạm vi loại giấy tờ được hỗ trợ và cách phân biệt revision/loại | `Nêu rõ` — Business rules, sheet "Thông tin theo loại giấy tờ": bảng field-by-field cho 6 loại (CCCD gắn chip, CCCD thường/White Card, CMND chip/thường/giấy, Hộ chiếu VN, Hộ chiếu nước ngoài) + "Validation OCR ver 2/3" thêm CMND Quân đội. | Thiết kế chính §3.1/§6.4 chỉ cho hai lựa chọn ở UI, `CCCD`/`Hộ chiếu`, tự nhận diện revision. CCCD chạy YOLO11 layout + RapidOCR PP-OCRv6; Passport chạy TD3 OCR + parser MRZ + toàn bộ check digit ICAO; M2 = `DONE`. `Implemented`/`Source-reviewed`. NFC/chip data đã `OUT_OF_SCOPE` MVP có ghi rõ ở thiết kế chính §3.2. CMND/military ID không có trong scope — hẹp hơn eyePass ở điểm này. |
...
| C02.7 | Fixture synthetic và benchmark có metric (OCR/MRZ) độc lập với session | `Không đề cập` — không có tài liệu bàn giao nào mô tả benchmark suite hay dataset registry cho OCR; các file kiểm thử chỉ là test case pass/fail thủ công. | Kế hoạch mô tả M4 khá chi tiết: CLI/registry riêng biệt session, dataset candidate (MIDV-2020, DocXPand-25k, MIDV-500/SmartDoc), metric cụ thể (field exact match, CER/WER, MRZ exact/check digit) — nhưng đó vẫn chỉ là mô tả trong tài liệu lập kế hoạch. `Conceptual`/`Self-reported`, không có CLI/registry/benchmark runner nào trong `backend/`. |

#### Status và nhận định

| ID | Status (V-ID) | Nhận định | Ưu tiên | Ghi chú |
|---|---|---|---|---|
| C02.1 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | P2 | Đáp ứng cho CCCD + Passport, đúng phạm vi MVP. CMND/military ID hẹp hơn eyePass, nhưng CMND đang bị thay thế bởi CCCD tại VN nên không tính là regression; NFC/chip đã loại khỏi scope có chủ đích. |
| C02.2 | `IN_SCOPE` | `GAP` (CCCD) / `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` (Passport MRZ) | **P1** | CCCD chưa có structured field extraction, xác nhận qua source, không ai gán lý do/owner/milestone riêng cho khoảng trống này. Vì CCCD là loại giấy tờ chính, cả M3 (quality theo field) lẫn M6 (hiển thị structured fields đã mask) đều đang chờ dữ liệu này. |
...
| C02.4 | `IN_SCOPE` | `GAP` | **P1** | eyePass nêu rõ với error code/message cụ thể; V-ID mới ở mức `Designed`, M3 chưa bắt đầu. Chưa tới lượt trên đường găng roadmap (M2 → M3) chứ không phải quyết định hoãn có chủ đích — ảnh hưởng trực tiếp trải nghiệm technical demo và là input cho M6. |
| C02.5 | `IN_SCOPE` | `GAP` | P2 | eyePass có đặc tả và test case rõ; V-ID chưa đề cập ở đâu cả. Edge case, không chặn happy path. |
| C02.6 | `IN_SCOPE`? — cần owner quyết định phạm vi | `GAP` (không phải `KHÔNG KẾ THỪA`) | P2 | eyePass cho end-user tự sửa trường OCR có validate; V-ID chưa nói rõ end-user có được tự sửa lúc capture hay không — §6.10.7 chỉ nói về reviewer. Chưa đủ căn cứ để biết đây là từ chối chủ đích hay đơn giản chưa thiết kế tới, nên đề xuất đưa vào decision log: đánh đổi thật là giảm ma sát capture so với rủi ro data subject "sửa" để né rule validate. |
| C02.7 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — THEO THIẾT KẾ` | P2 | M4 có đặc tả rõ nhưng chưa bắt đầu, self-reported only. Không cần P1 vì M4 hợp lý đứng sau M2/M3 trên đường găng, không chặn technical demo hiện tại. |

### 4. Domain C03 — Sinh trắc học & anti-spoof
...
| C03.2 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | `P2` | Source-reviewed đủ để đạt ngưỡng "kiểm chứng được", nhưng nên bổ sung test đặt tên riêng để lên `Hardened`. |
| C03.3 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không có active liveness. Năng lực V-ID tự thêm, có code và test, nhưng chưa benchmark/calibrate làm bằng chứng chống spoof production — xem D07 trong decision log, gộp chung với C04.5 vì cùng phụ thuộc M4/M5. |
| C03.4 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | `P2` | eyePass chỉ có mã lỗi "Face is spoof" không rõ cơ chế, không đủ cụ thể để coi là một yêu cầu — nhưng V-ID vẫn có 3 sub-signal thật, một phần đã test-verified. |
| C03.5 | `IN_SCOPE` (digit-challenge) / `DEFERRED_BY_DESIGN` (voice-spoof đúng nghĩa) | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không đề cập voice. V-ID hiện chỉ verify chuỗi 6 số qua ASR — tách rõ khỏi voice-spoof để không tuyên bố quá mức. |
| C03.6 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không đề cập lip-sync. V-ID có provider thật, fail-closed khi chưa cấu hình, nhưng chưa có test đặt tên riêng. |
...
### 5. Domain C04 — Decision & manual review
...
| C04.3 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không đề cập review queue. Core đã `Implemented`; assignment/idempotency/`ESCALATE` còn `Designed` (M6 chưa bắt đầu) — gap nội bộ, không liên quan eyePass. |
| C04.4 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P1` | eyePass không đề cập RBAC granularity. V-ID tự thêm, đặc tả rõ ở M6 nhưng chưa triển khai; ưu tiên trước MVP feature-complete vì M6 nằm trên đường găng roadmap. |
| C04.5 | `DEFERRED_BY_DESIGN` | `DEFERRED_BY_DESIGN` | `TBD` | V-ID để threshold production `TBD` có chủ đích. eyePass có ghi ngưỡng nhưng thiếu versioning/approval process — cần owner quyết định trước khi coi mục này xong. |

### 6. Domain C05 — API & integration
...
| C05.2 | API versioning theo path/header, không đổi ngầm ảnh hưởng integrator | `Không đề cập` — API doc liệt kê endpoint tuyệt đối, không tiền tố version; không tài liệu nào mô tả chính sách versioning. | Có version prefix `/api/v2` (`app.include_router(router, prefix=settings.api_prefix)`, `backend/app/main.py:35`), trích từ milestone M2. `Implemented`/`Source-reviewed`, nhưng chưa thấy chính sách deprecation/breaking-change cho version cũ. |
...
| ID | Status (V-ID) | Nhận định | Ưu tiên | Ghi chú |
|---|---|---|---|---|
| C05.1 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | — | Nguyên tắc "không lộ raw data" đã triển khai và source-reviewed; mở rộng field (`reason_codes`/`correlation_id`) là P2. |
| C05.2 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không có khái niệm versioning. V-ID có version prefix nhưng chưa xác nhận quy trình deprecation — gap nội bộ. |
| C05.3 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — THEO THIẾT KẾ` | `P1` | eyePass không đề cập idempotency. V-ID có thiết kế chi tiết nhưng chưa nối vào schema hiện tại — cần trước khi mở API cho hệ thống ngoài phụ thuộc retry an toàn. |
...
| C05.5 | `IN_SCOPE` | `GAP` | **P1** | `callback_url` đã expose cho integrator dùng nhưng dispatch thật chưa tồn tại, và không có lý do chủ đích nào được ghi nhận cho việc đó — nếu hệ thống ngoài bắt đầu dựa vào field này trước khi dispatch thật xong, đây là rủi ro tích hợp thật, không chỉ thiếu tính năng. |
...
### 7. Domain C06 — Model governance & offline
...
| ID | Status (V-ID) | Nhận định | Ưu tiên | Ghi chú |
|---|---|---|---|---|
| C07.1 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | `P2` | eyePass trả raw OCR/face-attribute chi tiết trong response; V-ID minimize theo contract M0 (đã xong) và code review, một phần test-verified (MRZ). P2: thêm test riêng cho các capability còn lại. |
| C07.2 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | — | eyePass không đề cập mã hóa/opaque key. Năng lực bảo mật V-ID tự thêm, đã hardened. |
| C07.3 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không đề cập purge/retention. Cơ chế chạy thật nhưng chưa có test đặt tên riêng — nên thêm test cascade-delete và idempotency (chạy hai lần liên tiếp) để lên `Hardened`. |
| C07.4 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không có khái niệm audit trail. Audit event ghi cho submit/purge/decide đã chạy nhưng chưa test-verified; audit cho xem/giải mã/export raw evidence là phần riêng của C04.4, đã P1 ở đó nên không lặp ưu tiên ở đây. |
| C07.5 | `DEFERRED_BY_DESIGN` | `DEFERRED_BY_DESIGN` | `TBD` | eyePass không đề cập retention matrix. Quyết định chủ đích của V-ID, cần Legal/DPO và business owner chốt — không phải gap. |

### 9. Domain C08 — Security & operations
...
| C08.5 | Quy trình incident response / change control | `Nêu chưa đầy đủ` — handover checklist liệt kê đầu mối Security và Compliance trong danh sách stakeholder (mục 1.1.2) và có nhắc "defect còn mở/đã đóng" như một phần bàn giao kiểm thử (mục 1.5.1), nhưng không có tài liệu quy trình incident response hay change control nào được đính kèm hay mô tả chi tiết. | Không tìm thấy runbook, quy trình incident response hay change-control gate nào trong repo. Kế hoạch chỉ nhắc "demo runbook hoàn chỉnh" như một next action còn mở của M7 — mới ở mức ý định. `Conceptual`/`Doc-only`. |

#### Status và nhận định

| ID | Status (V-ID) | Nhận định | Ưu tiên | Ghi chú |
|---|---|---|---|---|
| C08.1 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | P2 | Xác thực cho cả hai actor có quyền cao đã triển khai và source-reviewed — đơn giản nhưng đúng chức năng. eyePass có thêm bước root-detection cho SDK di động mà V-ID không cần vì không có SDK riêng. |
| C08.2 | `IN_SCOPE` | `GAP` | **P1** | `TOKEN_SECRET`/`EVIDENCE_KEY` fail-closed đúng, nhưng `VID_CLIENT_KEY`/`REVIEWER_TOKEN` fallback êm về giá trị mặc định công khai và không có gate nào theo `environment` để chặn việc đó ngoài development. Không tìm thấy quyết định chủ đích nào cho khoảng hở này — rủi ro thật nếu ai đó deploy ngoài máy dev mà quên set hai biến. |
| C08.3 | `IN_SCOPE` | `GAP` | P2 | Chưa rò rỉ thật, nhưng thiết kế đặt tên hiện tại là bẫy chờ sẵn cho lần sửa lỗi tiếp theo. Nên đổi tên hoặc bỏ hẳn biến `NEXT_PUBLIC_` này trước khi ai đó "sửa" sai cách. |
| C08.4 | `IN_SCOPE` | `GAP` | P2 | Không có access/error logging hay observability nào ngoài readiness endpoint. An toàn PII hiện tại là hệ quả của việc chưa log gì, không phải kiểm soát chủ động — cần thiết kế logging có chủ đích (kèm quy tắc redact) trước khi thêm log thật. |
| C08.5 | `IN_SCOPE` | `GAP` | P2 | Chưa có runbook hay quy trình incident/change-control nào, kể cả ở mức thiết kế. Hợp lý cho giai đoạn technical demo hiện tại, nhưng cần trước khi mở rộng ngoài demo nội bộ. |

### 10. Domain C09 — Quality & verification
...
| C09.4 | Formatter/linter/type-checker sạch cho toàn bộ codebase, theo đúng gate mà `PROJECT_ROADMAP.md` (M7) yêu cầu | `Không đề cập` — không có khái niệm formatter/linter/type-checker trong bộ tài liệu bàn giao (đây là công cụ phát triển nội bộ, không phải thứ một gói bàn giao cho khách hàng thường liệt kê). | `cd backend && uv run ruff check .` báo "All checks passed!" và `uv run ruff format --check .` báo 51 file đã đúng định dạng — formatter/linter `Implemented`/`Test-verified`. Nhưng `uv run mypy app ai_modules` trả về 19 lỗi trên 9 file: phần lớn là thiếu type stub cho thư viện ngoài (`vosk`, `onnxruntime`, `syncnet_python`, mức độ thấp), nhưng 6 lỗi nằm ngay trong `app/services/ekyc.py:63,309-321` — đúng vùng code của `purge_due()` mà C07.3 cũng ghi nhận thiếu test — và 3 lỗi khác ở `app/api.py:245,352,463` do cách SQLModel khai báo cột `created_at` mà mypy không suy luận đúng kiểu. Type-checker này `GAP`, không phải mơ hồ hay tùy nghi diễn giải: `PROJECT_ROADMAP.md` (M7) liệt kê "formatter, linter, type checker... pass" là điều kiện hoàn thành rõ ràng, và điều kiện đó hiện chưa đạt. Đáng nói thêm: một bản trước đây của báo cáo này trích số liệu "35 passed" từ tài liệu tự báo cáo — số thật khi tự chạy là 40. |
...
| C09.7 | Tài liệu acceptance-criteria / kịch bản chấp nhận có cấu trúc, theo dõi được trạng thái pass/fail | `Nêu rõ` — UAT checklist là một artifact acceptance-criteria khá chính quy: phân cấp Category → Function → Sub-function → Item, mỗi dòng có kết quả mong đợi cụ thể, dùng chung cho cả QC lẫn khách hàng ký nhận bàn giao. | Không có artifact acceptance-criteria độc lập. Điều kiện hoàn thành nằm rải rác trong `PROJECT_ROADMAP.md` dưới dạng bullet "Đầu ra và tiêu chí hoàn thành" cho từng milestone — M7 làm ví dụ có 10 bullet cụ thể, kiểm chứng được, nhưng không theo khuôn mẫu chuẩn hóa và không có cơ chế theo dõi pass/fail như bảng UAT của eyePass. `Designed`/`Doc-only`. |

#### Status và nhận định

| ID | Status (V-ID) | Nhận định | Ưu tiên | Ghi chú |
|---|---|---|---|---|
| C09.1 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | `P2` | eyePass không có khái niệm unit/contract test ở tầng kiến trúc. Kỷ luật kỹ thuật V-ID tự đặt ra, đã test-verified đầy đủ. |
...
| C09.4 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` (formatter/linter) · `GAP` (type-checker) | **P1** | Formatter/linter sạch, verified trực tiếp. Type-checker có 19 lỗi thật, một phần ngay trong code purge chưa có test (C07.3), và không ai giải thích vì sao mypy chưa sạch — đây là điều kiện M7 nêu rõ nên `GAP` chứ không phải hoãn có chủ đích. P1 vì đây là gate trước "integrated demo hardening". |
| C09.5 | `IN_SCOPE` | `GAP` | `P2` | Chưa có CI tự động, hiện dựa hoàn toàn vào kỷ luật thủ công. Chấp nhận được ở quy mô solo-dev/technical-demo hiện tại, nhưng nên nâng cấp trước khi nhiều người cùng đóng góp code. |
| C09.6 | `IN_SCOPE` | `GAP` | `P2` | Frontend chưa có test hành vi tự động, dù lint/typecheck sạch. Bề mặt UI còn nhỏ nên rủi ro chưa cao, nhưng nên bổ sung trước khi luồng capture/review phức tạp thêm. |
| C09.7 | `IN_SCOPE` | `KẾ THỪA CÓ CHỌN LỌC` | `P2` | Cấu trúc UAT checklist của eyePass (phân cấp category/function, cột kết quả mong đợi, theo dõi pass/fail/pending) là một khuôn mẫu đáng tham khảo khi V-ID hình thức hóa acceptance test cho M7 — không sao chép nội dung, chỉ tham khảo cấu trúc. |

### 11. Domain C10 — Delivery governance

| ID | Expectation | eyePass — Mức nêu trong đặc tả / Trích dẫn | V-ID-eKYC — Maturity / Confidence / Evidence |
|---|---|---|---|
| C10.1 | Roadmap thực thi có cấu trúc phụ thuộc rõ ràng giữa các milestone (không chỉ một danh sách phẳng) | `Không đề cập` — bộ tài liệu bàn giao là một checklist bàn giao cho sản phẩm đã hoàn thiện, không phải một roadmap thực thi hướng tới tương lai; không có tài liệu nào trong bộ bàn giao trình bày thứ tự phụ thuộc giữa các hạng mục. | `PROJECT_ROADMAP.md` §3 vẽ sơ đồ phụ thuộc tường minh: M0 làm nền cho cả M1 và nhánh M2→M3, M2 tiếp tục dẫn tới M4→M5→M6→M7 theo đường găng, còn hai workstream X1 (config/secret hardening) và X2 (dataset license/provenance) chạy song song ngoài đường găng chính. Mỗi milestone M0–M7 có mục tiêu, đầu ra, tiêu chí hoàn thành viết thành đoạn văn riêng, không chỉ là tên hạng mục. `Implemented`/`Source-reviewed`. |
| C10.2 | Release gate / definition of done cho lần đóng gói demo tiếp theo, gồm nhiều điều kiện cụ thể chứ không phải một dòng mô tả chung | `Nêu chưa đầy đủ` — checklist bàn giao có cột "Trạng thái" theo từng hạng mục (tất cả đang ghi "Đã bàn giao"), nên có tồn tại một hình thức theo dõi hoàn thành, nhưng không có nơi nào gộp các điều kiện đó thành một gate tổng hợp nhiều tiêu chí cho một lần release cụ thể. | M7 trong `PROJECT_ROADMAP.md` liệt kê 8 điều kiện phải đạt đồng thời: chạy end-to-end ba lần liên tiếp trên thiết bị chỉ định, quality gate route đúng mặt lỗi, đổi provider bằng config và chứng minh được fallback, mọi provider lỗi trả `UNAVAILABLE`, benchmark/threshold hiển thị đúng provenance, audit đầy đủ cho thao tác reviewer, test tải/resource/timeout, và formatter/linter/type-checker/test/model-verify/Docker-smoke đều pass trước khi purge dữ liệu rehearsal. Một gate nhiều điều kiện thật, `Implemented`/`Source-reviewed`. |
| C10.3 | Cơ chế theo dõi quyết định còn mở có cấu trúc: định danh riêng, trạng thái, owner đề xuất — không chỉ một danh sách gạch đầu dòng | `Không đề cập` — không có khái niệm "quyết định còn mở" nào xuất hiện trong bộ tài liệu bàn giao; đây là tài liệu bàn giao một sản phẩm đã xong, không phải tài liệu quản lý quyết định đang chờ. | Trước đợt đánh giá này, V-ID đã có thói quen ghi lại quyết định còn mở nhưng rải rác ở hai nơi: `AGENTS.md` mục "Các quyết định còn mở" liệt kê 7 câu hỏi lớn về production (lawful basis, retention, region, KMS, threshold, SLA, quyền thao tác evidence), còn `PROJECT_ROADMAP.md` §7 liệt kê 6 câu hỏi khác thiên về thực thi (nơi lưu benchmark data, target false-recapture rate, quyền reviewer unmask, thời hạn disclosure grant...). Cả hai chỉ là danh sách gạch đầu dòng, không ID, không trạng thái, không owner theo từng mục — `Designed`/`Source-reviewed` cho phần nền tảng này. Bản decision log có cấu trúc đầy đủ (D01–D09, trạng thái `OPEN`/`AWAITING_OWNER`/`DECIDED`/`CLOSED`, owner riêng từng dòng) chính là Phần E của báo cáo này — sản phẩm phụ của đợt đánh giá, không phải năng lực có sẵn từ trước của V-ID, nên không tính là bằng chứng năng lực độc lập. |
...
| C10.2 | `IN_SCOPE` | `ĐÃ ĐÁP ỨNG — KIỂM CHỨNG ĐƯỢC` | — | eyePass theo dõi hoàn thành theo từng hạng mục nhưng không gộp thành gate nhiều điều kiện; V-ID có gate M7 tám điều kiện, đã xác nhận nội dung. |
| C10.3 | `IN_SCOPE` | `NGOÀI YÊU CẦU EYEPASS` | P2 | eyePass không có khái niệm quyết định còn mở. V-ID đã ghi nhận nhưng rải rác, không ID/trạng thái/owner theo từng mục — đáng gộp `AGENTS.md` và `PROJECT_ROADMAP.md` §7 vào một nơi duy nhất, nhưng không cấp bách vì decision log có cấu trúc đã bắt đầu tồn tại qua chính đợt đánh giá này. |
...
| C10.5 | `IN_SCOPE` | `GAP` | P2 | eyePass có bản đồ vai trò/đầu mối liên hệ cụ thể; V-ID hiện chỉ có một owner chung cho mọi quyết định. Hợp lý ở quy mô một người/đội nhỏ làm technical demo, nhưng không có ghi chú quản trị nào chủ động hoãn việc này nên xếp `GAP` chứ không phải hoãn có chủ đích — cần xử lý trước khi mở rộng đội hoặc chuyển sang pilot nhiều bên liên quan hơn. |

Không có `P0`/`P1` trong domain này. Đáng nói minh bạch: repo hiện không có `CODEOWNERS`, không template PR/issue, không cấu hình CI — nhưng phần đó thuộc phạm vi C08 (đã xử lý ở đó) nên không lặp lại. Lịch sử `git log` cho thấy commit message không theo quy ước cố định, phù hợp giai đoạn technical demo một người làm chính, không phải phát hiện bất ngờ.

### 12. Việc cần làm tiếp (đề xuất, chưa phải quyết định)

**P1 đang mở, cần owner xác nhận mức ưu tiên tương đối (Phần E §2):**

1. C05.5 — webhook dispatch (signature/retry/replay) chưa tồn tại, chỉ có field lưu trữ + allowlist.
2. C04.4 — RBAC granularity cho decrypt/export/delete/correct đã thiết kế (M6) nhưng chưa triển khai.
3. C02.2 — CCCD chưa có structured field extraction (M3/M6 phụ thuộc dữ liệu này).
4. C02.4 — document quality gate/recapture (M3) `NOT_STARTED`, ảnh hưởng trải nghiệm technical demo.
5. C05.3 — idempotency key đã thiết kế (§6.2/§11.2) nhưng chưa nối vào `CreateSessionRequest`.
6. C08.2 — `VID_CLIENT_KEY`/`REVIEWER_TOKEN` fallback về giá trị mặc định công khai, không có gate theo `environment`.
7. C09.4 — `mypy` có 19 lỗi thật (một phần trong `purge_due()`, cùng vùng code C07.3 đã thiếu test), vi phạm gate M7.

**Quyết định phạm vi cần owner (không phải ưu tiên kỹ thuật, mà là câu hỏi "có muốn làm không"):**

8. C02.6 — end-user có được tự sửa/skip-validate trường OCR trước khi submit không (eyePass có; V-ID chưa nói rõ đây là `GAP` hay chủ đích không theo).
9. Nhóm C03.2–C03.6 + C04.5 — các tín hiệu sinh trắc học nâng cao và threshold đều tự nhận "chưa benchmark/calibrate thành bằng chứng production", cùng phụ thuộc M4/M5 (`NOT_STARTED`) — nên xử lý như một cụm quyết định, không rải rác từng mục.
10. C06.4, C07.5 — dataset registry record và retention matrix production đều `DEFERRED_BY_DESIGN` có chủ đích, cần mốc thời gian quyết định rõ hơn "TBD".
11. C10.5 — có nên phân vai trò owner theo chức năng (security/compliance/AI...) trước khi mở rộng đội, hay giữ một owner chung tới khi nào.

**Còn lại:**

12. Trước khi công bố kết luận cho stakeholder ngoài, cần bước calibration với assessor thứ hai theo rubric §1 — chưa thực hiện trong bản draft này. Toàn bộ 10 domain (C01–C10) hiện đã có dòng chính thức trong matrix này (56 dòng capability).

---

## Phần E — Risk & Decision Log

### 1. Cách đọc bảng

Trạng thái đi theo chuỗi `OPEN` (chưa ai xử lý) → `AWAITING_OWNER` (đã trình, chờ quyết định) → `DECIDED` (owner đã chốt, ghi rõ quyết định + ngày) → `CLOSED` (đã triển khai xong). Cột Loại phân biệt `PRIORITY` — kỹ thuật đã rõ, chỉ cần xếp lịch làm cái gì trước — với `SCOPE`, tức câu hỏi "có làm hay không" cần quyết định sản phẩm/pháp lý chứ không chỉ kỹ thuật. Cột Dòng ma trận trỏ ngược về ID trong Phần D; mở dòng đó ra để xem đầy đủ evidence trước khi quyết định gì.

### 2. Quyết định ưu tiên (`PRIORITY`) — P1 đang mở

| ID | Chủ đề | Bối cảnh ngắn | Dòng ma trận | Trạng thái | Owner đề xuất |
|---|---|---|---|---|---|
| D01 | Webhook dispatch (signature/retry/replay) | `callback_url` hiện chỉ là field lưu trữ + allowlist check — **không có lệnh gọi HTTP ra ngoài nào trong source**. Nếu hệ thống ngoài bắt đầu tích hợp dựa trên field này trước khi dispatch thật tồn tại, đây là rủi ro tích hợp thật, không chỉ thiếu tính năng. | C05.5 | `OPEN` | Backend |
| D02 | RBAC granularity cho reveal/decrypt/export/delete evidence | Đã có đặc tả đầy đủ ở milestone M6 (quyền `pii:unmask`/`evidence:export`/... tách riêng, audit từng lần cấp/xem), nhưng implementation hiện tại chỉ có một role `require_reviewer` phẳng. M6 nằm trên đường găng roadmap trước MVP feature-complete. | C04.4 | `OPEN` | Backend, Security/Privacy |
| D03 | CCCD chưa có structured field extraction | Passport đã có; CCCD (loại giấy tờ chính tại VN) demo chỉ trả metadata OCR/layout, chưa lưu field có cấu trúc (`id_number`, `date_of_birth`...). M3 (quality gate theo field) và M6 (hiển thị structured field đã mask) đều phụ thuộc dữ liệu này. | C02.2 | `OPEN` | AI, Backend |
| D04 | Document quality gate & reason-code recapture (M3) | Đặc tả đầy đủ (blur/glare/corner/brightness/contrast/occlusion, reason code, state `RECAPTURE_DOCUMENT`) nhưng M3 = `NOT_STARTED`, capability `document_quality` chưa có provider đăng ký. Ảnh hưởng trực tiếp trải nghiệm technical demo. | C02.4 | `OPEN` | AI, Product |
| D05 | Idempotency key cho state-changing request | Thiết kế chi tiết ở §6.2/§11.2 (dedupe theo actor+endpoint+request hash) nhưng chưa nối vào `CreateSessionRequest` hiện tại. Cần trước khi mở API cho hệ thống ngoài phụ thuộc retry an toàn. | C05.3 | `OPEN` | Backend |
| D10 | Secret xác thực fallback về giá trị mặc định công khai | `TOKEN_SECRET`/`EVIDENCE_KEY` đã fail-closed đúng (`${VAR:?...}` trong `compose.yml`), nhưng `VID_CLIENT_KEY`/`REVIEWER_TOKEN` dùng `${VAR:-default}` — container khởi động bình thường với giá trị mặc định đã công khai trong `.env.example` nếu không set. `env/.env.local` hiện tại xác nhận `VID_CLIENT_KEY` vẫn còn nguyên giá trị mặc định. Không có gate nào dựa trên `Settings.environment` để chặn việc này ngoài development. | C08.2 | `OPEN` | Backend, Security/Privacy |
| D11 | `mypy` chưa sạch — 19 lỗi type thật | `PROJECT_ROADMAP.md` (M7) liệt kê type-checker sạch là điều kiện hoàn thành rõ ràng. Chạy trực tiếp `cd backend && uv run mypy app ai_modules` ra 19 lỗi trên 9 file — 6 lỗi nằm ngay trong `purge_due()` (`app/services/ekyc.py:63,309-321`, cùng vùng code D-worthy vì C07.3 cũng ghi nhận thiếu test ở đây), 3 lỗi ở khai báo cột SQLModel trong `api.py`, còn lại là thiếu type stub cho thư viện ngoài (mức độ thấp hơn). | C09.4 | `OPEN` | Backend |

**Đề xuất xếp lịch tương đối** (không phải quyết định — chỉ gợi ý dựa trên đường găng roadmap đã ghi trong kế hoạch): D01 và D02 chặn trực tiếp trước khi mở tích hợp/RBAC cho bên ngoài; D03 chặn cả D04 lẫn phần hiển thị structured field của D02; D04 và D05 có thể song song. D10 và D11 xác nhận được ngay trong vài phút (chạy lại đúng lệnh đã dùng để tìm ra chúng) nên có thể xử lý sớm song song với các mục khác, không cần chờ lịch trình. Owner xác nhận thứ tự thật trước khi đưa vào sprint planning.

### 3. Quyết định phạm vi (`SCOPE`) — cần quyết định sản phẩm/pháp lý, không chỉ kỹ thuật

| ID | Chủ đề | Bối cảnh ngắn | Dòng ma trận | Trạng thái | Owner đề xuất |
|---|---|---|---|---|---|
| D06 | End-user có được tự sửa/skip-validate trường OCR trước khi submit? | eyePass cho phép (2 config flag, ~40 test case field-level, có cả case chặn giá trị phi lý). V-ID chưa thiết kế tới; nguyên tắc gần nhất (§6.10.7) chỉ nói về **reviewer** correction, không nói end-user. Đánh đổi thật: giảm ma sát capture vs. rủi ro data subject "sửa" để né rule validate. Chưa đủ căn cứ để biết đây là chủ đích không theo eyePass hay đơn giản chưa thiết kế tới. | C02.6 | `OPEN` | Product, Security/Privacy |
| D07 | Cụm benchmark/calibration cho threshold + tín hiệu sinh trắc học nâng cao | Threshold face-match (C04.5) và các signal active liveness/deepfake/replay/camera-injection/voice/lip-sync (C03.2–C03.6) đều tự nhận **chưa benchmark/calibrate thành bằng chứng chống spoof production**. Cả hai cùng phụ thuộc M4 (benchmark foundation) và M5 (threshold lifecycle), cả hai `NOT_STARTED`. Đề xuất xử lý như một quyết định gộp thay vì rải rác từng mục, vì chúng chia sẻ cùng điều kiện tiên quyết. | C04.5, C03.2–C03.6 | `OPEN` | AI, Product |
| D08 | Mốc thời gian cho dataset registry record (workstream X2/M4) | Schema/quy trình 5 bước đã chốt (M0 `DONE`), nhưng dataset record cụ thể (nguồn, license, sensitivity) chưa có mốc rõ hơn "TBD". | C06.4 | `OPEN` | AI, Legal |
| D09 | Retention matrix production (thời hạn lưu theo data class × outcome) | Bảng 8 data class × 4 outcome đã thiết kế, phần lớn ô còn `TBD`. Cần Legal/DPO + business owner chốt trước khi coi C07 là "đủ" cho production. | C07.5 | `OPEN` | Legal/DPO, Product |
| D12 | Phân công owner theo vai trò chức năng hay giữ một owner chung? | eyePass có bản đồ vai trò/đầu mối cụ thể (PO/BA/Tech Lead/Security/Compliance...); V-ID hiện chỉ có "người dùng là owner cuối cùng" cho mọi quyết định. Hợp lý ở quy mô một người/đội nhỏ hiện tại, nhưng cần quyết định mốc nào thì phân vai trò — trước khi mở rộng đội hay chuyển sang pilot có nhiều bên liên quan hơn. | C10.5 | `OPEN` | Product |

### 4. Theo dõi P2 (không chặn, không cần quyết định gấp)

Danh sách đầy đủ nằm trong ma trận (mỗi dòng đã có ưu tiên `P2` riêng) — chỉ liệt kê ở đây những gì có khả năng trở thành quyết định thật nếu quy mô thay đổi:

- **C01.5** — hướng dẫn thời gian thực khi chụp selfie (V-ID mới `Conceptual`, eyePass đặc tả rất chi tiết) — có thể ảnh hưởng tỉ lệ capture thành công nếu mở rộng ngoài technical demo.
- **C02.5** — phát hiện nhiều giấy tờ trong khung hình — edge case, không chặn happy path.
- **C02.1** — mở rộng hỗ trợ CMND/military ID nếu owner muốn phạm vi rộng hơn eyePass.
- **C08.3** — đổi tên hoặc bỏ biến `NEXT_PUBLIC_VID_CLIENT_KEY` trước khi ai đó "sửa lỗi" bằng cách set nó thành giá trị thật.
- **C08.4** — cần thiết kế logging/observability có chủ đích (kèm quy tắc redact PII) trước khi thêm log thật, không phải sau.
- **C08.5** — viết runbook incident/change-control tối thiểu trước khi mở rộng ngoài technical demo nội bộ.
- **C09.5/C09.6** — thêm CI tự động và test hành vi cho frontend trước khi nhiều người cùng đóng góp code hoặc luồng capture/review phức tạp thêm.
- **C10.3** — gộp hai danh sách quyết định-còn-mở không cấu trúc (`AGENTS.md`, `PROJECT_ROADMAP.md` §7) vào decision log có cấu trúc này; khi có quyết định mới, ghi thẳng vào đây thay vì thêm gạch đầu dòng ở hai nơi cũ.

### 5. Quy trình cập nhật

Khi owner quyết định một item `PRIORITY`/`SCOPE`, đổi Trạng thái sang `DECIDED` và thêm một dòng ghi ngày, tên người quyết định, tóm tắt quyết định ngay dưới bảng tương ứng — không xóa lịch sử cũ. Khi đã triển khai xong theo quyết định đó thì chuyển sang `CLOSED`, đồng thời cập nhật dòng tương ứng trong ma trận (maturity/status) để hai tài liệu không bị lệch nhau. Đây là decision log có cấu trúc đầu tiên của dự án (xem C10.3), nên quyết định mới phát sinh ngoài phạm vi C01–C10 nên thêm thẳng vào đây theo Loại, thay vì quay lại ghi rải rác ở `AGENTS.md` hay `PROJECT_ROADMAP.md` §7 như trước.
