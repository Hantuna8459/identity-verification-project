...
| M4 | Benchmark foundation và dataset intake | `IN_PROGRESS` | `backend/benchmark/` là package độc lập, không import `app.core.database`/session state: `dataset_registry.py` load/validate record theo đúng schema `M0_CONTRACT_GOVERNANCE_BASELINE.md` §6 và fail-closed nếu `approval_status != approved_for_evaluation` (`test_dataset_registry_fails_closed_on_unapproved_dataset`); `datasets/registry.json` có hai dataset `internal_synthetic`/`approved_for_evaluation` cho `document_ocr` và `passport_mrz`, sinh runtime từ `fixtures/{ocr_fixtures.py,mrz_fixtures.py}` (không tải, không commit dữ liệu thật); `quarantine.py` đọc thẳng `models/manifest.json` và tự động gắn bất kỳ candidate model nào đang `quarantined`/`rejected` vào `skipped_models` mỗi lần benchmark chạy (test riêng dùng manifest giả để khoá hành vi này, không phụ thuộc trạng thái thật đang đổi); `runners/document_ocr_runner.py` chạy cả `LocalOcr` (RapidOCR PP-OCRv6, provider chính) lẫn `VietocrEngine` (`benchmark/engines/vietocr_engine.py`, candidate) trên cùng fixture, đo CER/WER cho cả hai; `runners/passport_mrz_runner.py` chạy `inspect_td3` thật đo detection accuracy (Wilson CI) và exact-match trên mẫu hợp lệ lẫn mẫu cố tình sai check digit; `cli.py` (`python -m benchmark.cli run --capability all`) ghi report JSON có device/dataset/latency p50-p95 vào `backend/benchmark/reports/` (gitignored). `vietocr-vgg-transformer`: owner đổi sang `evaluation_only`/`usage_scope: [benchmark_only]` (2026-08-11, chấp nhận rủi ro trước pilot vì M2 cho phép đổi provider không cần redesign, xem `approval_reference` trong manifest - license vẫn `WEIGHT_LICENSE_REVIEW_REQUIRED` chưa review); weight tải qua `scripts/models.py --profile benchmark_only` (checksum-verified, không tự tải lúc benchmark chạy), config `vgg_transformer` fetch một lần rồi lưu local (`models/vietocr/vgg_transformer_config.yml`) vì `Cfg.load_config_from_name()`/`Predictor()` gốc tự gọi mạng - vi phạm rule M0 "adapter không được tự download runtime" nếu dùng trực tiếp. Dependency `vietocr`/`setuptools<81` chỉ nằm trong `[dependency-groups] dev` nên `Dockerfile`'s `uv sync --frozen --no-dev` không cài vào image. Kết quả so sánh thật đầu tiên (20 sample synthetic, có dấu tiếng Việt qua font DejaVu Sans - lần đầu render ASCII không dấu ra so sánh sai lệch, đã sửa): RapidOCR CER 5.4%/WER 19.4%; vietocr CER 7.1%/WER 25.8% nhưng nhanh hơn per-line (vietocr chỉ là recognizer, không có bước detect như RapidOCR). Kiểm chứng: `backend/tests/test_benchmark.py` (12 test, gồm fail-closed, quarantine flag trên manifest giả, xác định fixture, candidate_engines shape); `uv run pytest` (52 passed); `uv run ruff check/format` và `uv run mypy benchmark` sạch (PIL/vietocr báo import-untyped, cùng loại với onnxruntime đã chấp nhận sẵn trong repo) | Mới phủ 2/9 capability trong bảng candidate dataset của roadmap (document_ocr, passport_mrz) và chỉ bằng synthetic smoke set (text render sạch, có dấu, không phải ảnh CCCD/passport chụp thật) nên vẫn có khoảng cách lớn với production; chưa có split identity/document/device (chỉ có `synthetic_smoke`); RapidOCR đang thắng vietocr trên test này nhưng biên độ hẹp (5.4% vs 7.1% CER) - không đủ để kết luận chắc, cần test trên ảnh CCCD/passport thật (mờ/lóa/góc nghiêng) mới có câu trả lời production; MIDV/DocXPand/OULU-NPU/FaceForensics++ và các dataset ngoài còn lại chưa bắt đầu license review, FaceForensics++ ghi rõ `external evaluation` only trong roadmap nên không tự động áp dụng logic "chấp nhận rủi ro" như vietocr | Benchmark RapidOCR vs vietocr trên fixture CCCD/passport thật (không phải text render sạch) để có câu trả lời production thay vì chỉ synthetic; face-match/passive-liveness/deepfake/voice-challenge cần dataset thật (DigiFace-1M/OULU-NPU/FaceForensics++, hoặc audio thật cho voice - không có TTS local để tự sinh) nên chỉ dựng scaffold report+metric trước, không tự chế dữ liệu giả để tránh cho ra điểm số gây hiểu lầm; quyết định license review FaceForensics++/1:N dedup vẫn chờ owner |
