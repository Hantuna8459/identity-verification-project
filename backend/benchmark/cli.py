...
from benchmark.runners import document_ocr_runner, passport_mrz_runner
...
_RUNNERS = {
    "document_ocr": lambda model_dir, seed: document_ocr_runner.run(model_dir, seed=seed),
    "passport_mrz": lambda model_dir, seed: passport_mrz_runner.run(seed=seed),
}


def _run_one(capability: str, model_dir: Path, seed: int, out_dir: Path) -> Path:
    result = _RUNNERS[capability](model_dir, seed)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for capability in capabilities:
        path = _run_one(capability, args.model_dir, args.seed, args.out)
