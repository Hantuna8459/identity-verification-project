    ...
    def analyze(
        ...
        for evidence_type in document_types:
            ...
            if passport:
                ...
                document_result["layout"] = self._run(
                    "document_layout", DocumentLayoutRequest(entry.payload)
                )
        ...
        try:
            ...
            liveness_result = self._run(
                "passive_liveness",
                PassiveLivenessRequest(image=live_image, bbox=live_face.bbox),
            )
            deepfake_result = self._run(
                "visual_deepfake",
                VisualDeepfakeRequest(image=live_image, bbox=live_face.bbox),
            )
